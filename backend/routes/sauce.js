...
router.post('/', limiter.modify, auth, multer, sauceControllers.createSauce); // recuperation de l'url du post et du contenue post (createSauce(objet body))
router.post('/:id/like', limiter.modify, auth, sauceControllers.likeSauce);
//Update (mettre a jour)
router.put('/:id', limiter.modify, auth, multer, sauceControllers.modifySauce); // "/"(adresse) + id  en paramettre de route , chemin du contenue la route
...
router.get('/:id', limiter.get , auth, sauceControllers.getOneSauce); // l'url du post et du contenue post (getOneSauce(objet body)) , un seul objet
router.get('/', limiter.get , auth, sauceControllers.getAllSauce);//url viser (route a contacter) par l'application route http://localhost:3000  ,  recupere  req(s)  get de tous le objet .
